...
def starts_with_a(array):
    return [x for x in array if x[0] == 'a']
...
def starts_with_a_vowel(array):
    return [x for x in array if x[0] in 'aeiou']
...
def greater_than_5(array):
    return [x for x in array if x > 5]
...
def all_above(array, num):
    return all(x > num for x in array)
...
def where_value_below(dictionary, number):
    return {k:v for k,v in dictionary.items() if v < number}
...
def where_key_starts_with(dictionary, letter):
    return {k:dictionary[k] for k in dictionary if k[0] == letter }
