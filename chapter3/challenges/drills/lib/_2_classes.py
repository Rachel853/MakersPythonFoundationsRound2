...
class Cohort():
    ...
    def list_employed_by(self, employer):
        return list(filter(lambda student: (student['employer'] == employer), self.student_list))
...
class Person():
    ...
    def get_work_address(self):
        address = self.dictionary['addresses'][0]
        return f"{address['building']} {address['street']}"
    
    def get_home_address(self):
        address = [f"{dictionary['building']} {dictionary['street']}" for dictionary in self.dictionary['addresses'] if dictionary['name'] == 'home']
        return address[0]
    
    def get_pets(self):
        pets_list = self.dictionary['pets']
        number_pets = len(pets_list)
        pet_text_bites = [f" a {pet_dictionary['animal']} called {pet_dictionary['name']}" for pet_dictionary in pets_list]
        return f"{self.dictionary['name']} has {number_pets} pets:" + ','.join(pet_text_bites)