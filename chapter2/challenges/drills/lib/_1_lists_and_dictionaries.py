...
def the_beatles():
    return "john paul george ringo".split()
...
def new_band_member(dictionary):
    ...
    # band.update(dictionary)
    # return band

    # OTHER OPTION 2
    # return {"vocalist": "miss piggy", "lead_guitar": "scooter"} | dictionary 

    # OTHER OPTION 3
    band = {**{"vocalist": "miss piggy", "lead_guitar": "scooter"}, **dictionary}
    return band
...
def remove_nones_from_dictionary(dictionary):
    # Option 1 
    # return {k:dictionary[k] for k in dictionary if dictionary[k] != None}

    # Option 2
    return {k:v for k,v in dictionary.items() if v != None}
